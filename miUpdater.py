...
def shutdownDB(parent, mgr, ids, on_done, client):
    ...
    if addonId in ids and hasattr(mw, "MigakuChinese"):
        miInfo(
            "Migaku Chinese's database will be diconnected so that the update may proceed. The add-on will not function properly until Anki is restarted after the update."
        )
...
def restartDB(*args):
    if addonId in dledIds and hasattr(mw, "MigakuChinese"):
        miInfo(
            "Migaku Chinese has been updated. Migaku Chinese will not function properly until Anki is restarted. Please restart Anki to start using the new version now!"
        )
