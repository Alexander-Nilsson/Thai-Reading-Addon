...
def init_xclip_clipboard():
    ...
    def paste_xclip(primary=False):
        ...
        if primary:
            selection = PRIMARY_SELECTION
        p = subprocess.Popen(
            ["xclip", "-selection", selection, "-o"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True
        )
        stdout, _stderr = p.communicate()
...
def init_klipper_clipboard():
    def copy_klipper(text):
        text = _stringifyText(text)  # Converts non-str values to str.
        p = subprocess.Popen(
            ["qdbus", "org.kde.klipper", "/klipper", "setClipboardContents", text.encode(ENCODING)],
    ...
    def paste_klipper():
        p = subprocess.Popen(
            ["qdbus", "org.kde.klipper", "/klipper", "getClipboardContents"], stdout=subprocess.PIPE, close_fds=True
        )
        stdout, _stderr = p.communicate()
...
def set_clipboard(clipboard):
    ...
    if clipboard not in clipboard_types:
        raise ValueError("Argument must be one of {}".format(", ".join([repr(_) for _ in clipboard_types.keys()])))
