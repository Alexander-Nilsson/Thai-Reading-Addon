...
@pytest.fixture
def config():
    return AddonConfig(_raw={"ReadingType": "ipa", "FontSize": 100})
...
class TestConfigDelta:
    ...
    def test_to_dict_reading_type(self):
        d = ConfigDelta(reading_type="ipa").to_dict()
        assert d == {"ReadingType": "ipa"}

    def test_to_dict_tuple_to_list(self):
        d = ConfigDelta(thai_tones=("#fff", "#000", "#111", "#222", "#333")).to_dict()
        assert d["ThaiTones"] == ["#fff", "#000", "#111", "#222", "#333"]
    ...
    def test_from_dict_round_trip(self):
        raw = {
            "ReadingType": "ipa",
            "FontSize": 90,
            "ThaiTones": ["red", "green", "blue", "yellow", "gray"],
            "ActiveFields": ["hover;all;Basic;Card 1;Front;Both"],
        }
        delta = ConfigDelta.from_dict(raw)
        assert delta.reading_type == "ipa"
        assert delta.font_size == 90
        assert delta.thai_tones == ("red", "green", "blue", "yellow", "gray")
    ...
    def test_from_dict_to_dict_round_trip(self):
        original = ConfigDelta(
            ...
            active_fields=("hover;all;Basic;Card 1;Front;Both;rtgs",),
        )
        ...
        assert restored.thai_tones == original.thai_tones
...
class TestValidate:
    ...
    def test_rejects_wrong_thai_tone_count(self, mutation):
        errs = mutation.validate(ConfigDelta(thai_tones=("#000", "#111")))
        assert len(errs) == 1
        assert errs[0].field == "thai_tones"
    ...
    def test_allows_active_field_with_missing_note_type(self, mutation):
        errs = mutation.validate(ConfigDelta(active_fields=("hover;User 1;MissingNote;Card 1;Front;Both;rtgs",)))
        assert errs == []

    def test_allows_active_field_with_missing_card_type(self, mutation):
        errs = mutation.validate(ConfigDelta(active_fields=("hover;User 1;Basic;MissingCard;Front;Both;rtgs",)))
        assert errs == []

    def test_allows_active_field_with_missing_field(self, mutation):
        errs = mutation.validate(ConfigDelta(active_fields=("hover;User 1;Basic;Card 1;MissingField;Both;rtgs",)))
        assert errs == []

    def test_accepts_valid_active_field(self, mutation):
        errs = mutation.validate(ConfigDelta(active_fields=("hover;User 1;Basic;Card 1;Front;Both;rtgs",)))
        assert errs == []
...
class TestSaveConfig:
    def test_saves_and_returns_config(self, mutation, anki):
        result = mutation.save_config(ConfigDelta(reading_type="ipa"))
        assert result.reading_type == "ipa"
        assert anki.written["test-addon"]["ReadingType"] == "ipa"
    ...
    def test_merge_preserves_other_keys(self, mutation, anki):
        ...
        assert result.reading_type == "ipa"  # from original config
    ...
    def test_merge_fills_from_defaults(self, mutation, anki):
        ...
        assert result.reading_type == "ipa"  # from config
    ...
    def test_updates_internal_config(self, mutation):
        mutation.save_config(ConfigDelta(reading_type="rtgs"))
        assert mutation._config.reading_type == "rtgs"
